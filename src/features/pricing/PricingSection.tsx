      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16 space-y-4">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full text-xs font-semibold bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20">
            ...
            <span>{t('pricing.badge', 'Transparent Project Pricing')}</span>
          </div>
          <h2 className="text-3xl sm:text-5xl font-extrabold font-heading text-slate-900 dark:text-white tracking-tight">
            {t('pricing.title', 'Flexible Freelance &')}{' '}
            <span className="gradient-text-primary">{t('pricing.highlight', 'Consultancy')}</span>
          </h2>
          <p className="text-slate-600 dark:text-slate-400 text-base sm:text-lg">
            {t('pricing.subtitle', 'Fixed-rate packages engineered for speed, clean architecture, and total transparency.')}
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          {PRICING_TIERS.map((tier, idx) => {
            const features = (t(`pricing.items.${tier.id}.features`, { returnObjects: true }) as string[]) || tier.features;
            ...
            return (
              ...
              >
                <div>
                  {tier.popular && (
                    <span className="absolute -top-3.5 left-1/2 -translate-x-1/2 px-4 py-1 rounded-full text-xs font-bold bg-gradient-to-r from-blue-600 to-violet-600 text-white shadow-lg flex items-center gap-1.5 whitespace-nowrap">
                      <Sparkles className="w-3.5 h-3.5" />
                      {t('pricing.mostPopular', 'Most Popular Choice')}
                    </span>
                  )}

                  <div className="mb-6 pt-2">
                    <h3 className="font-heading font-bold text-xl text-slate-900 dark:text-white">
                      {t(`pricing.items.${tier.id}.name`, tier.name)}
                    </h3>
                    <p className="text-xs text-slate-500 dark:text-slate-400 mt-1 leading-relaxed">
                      {t(`pricing.items.${tier.id}.description`, tier.description)}
                    </p>
                  </div>
                  ...
                  <div className="flex items-baseline gap-1 mb-6 pb-6 border-b border-slate-200 dark:border-slate-800">
                    ...
                    <span className="text-xs text-slate-500 dark:text-slate-400 font-semibold">
                      /{t('pricing.perProject', tier.period)}
                    </span>
                  </div>
                ...
                >
                  {t(`pricing.items.${tier.id}.ctaText`, tier.ctaText)}
